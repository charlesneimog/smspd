...
void *smsanal_childthread(void *zz)
{
        ...
        while(x->iDoAnalysis > 0)
        {
                ...
                {
                        if ((x->iSample - x->anal_params.sizeNextRead) >= 0)
                                sizeNewData = x->anal_params.sizeNextRead;
                        else
                ...
                {
                        x->iSample += sizeNewData;
                        if((x->iSample + x->anal_params.sizeNextRead) < x->nSamples)
                                sizeNewData = x->anal_params.sizeNextRead;
                        else
		...
		/* get one frame of sound */
                ...
                {
                        if (sms_getSound (&x->soundHeader, sizeNewData,  x->pSoundData, x->iSample) < 0)
                        {
		...
		x->iStatus = sms_analyze (sizeNewData, x->pSoundData, &x->smsbuf->smsData[x->iFrame],
                                          &x->anal_params);
        ...
        x->smsbuf->smsHeader.fResidualPerc = x->anal_params.fResidualAccumPerc / x->iFrame;
        x->smsbuf->ready = 1;
...
static void smsanal_soundfile(t_smsanal *x, t_symbol *filename)
{
	...
	/* open soundfile */
	if(sms_openSF (fullname->s_name , &x->soundHeader))
	{
                pd_error(x,"error in sms_openSF: %s", sms_errorString());
                return;
	}	    
        else if(x->verbose) post("reading soundfile %s", fullname->s_name);

        x->nSamples = x->soundHeader.nSamples;
        sms_initAnalysis (&x->anal_params, &x->soundHeader);

        /* fill sms header information  incase the analysis is written to file (by smsbuf) */
	sms_fillHeader (&x->smsbuf->smsHeader, &x->anal_params, "smsanal (smspd)");

        /* tell smsbuf how many frames (is this necessary?) */
	x->smsbuf->nframes = x->anal_params.nFrames;
        if(x->verbose)
        {
        ...
        /* allocate memory for frames to be analyzed */
...
static void smsanal_array(t_smsanal *x, t_symbol *arrayname, t_float samplerate)
{
        ...
        x->soundHeader.nSamples = x->nSamples;
        x->soundHeader.iSamplingRate = (int)samplerate;
        x->soundHeader.channelCount = 1;
        x->soundHeader.iReadChannel = 0;
